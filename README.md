...
<h1 align="center">CT Cloud .Net Software Development Kit (.Net SDK)</h1>

The CT Cloud .Net SDK allows you to easily work with CT Cloud services such as Elastic Compute Service (ECS) and
Virtual Private Cloud(VPC) without the need to handle API related tasks.

This document introduces how to obtain and use CT Cloud .Net SDK.

## Requirements

- To use CT Cloud .NET SDK, you must have CT Cloud account as well as the Access Key (AK) and Secret key (SK) of the
  CT Cloud account. You can create an Access Key in the CT Cloud console.

- To use CT Cloud .NET SDK to access the APIs of specific service, please make sure you do have activated the service 
  in [CT Cloud console](https://console.hc.sbercloud.ru/) if needed.
...
You must install `CTCloud.SDK.Core` library no matter which product development kit you need to use. Take using VPC
SDK for example, you need to install `CTCloud.SDK.Core` library and `CTCloud.SDK.Vpc` library:

- Use .NET CLI

``` bash
dotnet add package CTCloud.SDK.Core
dotnet add package CTCloud.SDK.Vpc
```

- Use Package Manager

``` bash
Install-Package CTCloud.SDK.Core
Install-Package CTCloud.SDK.Vpc
```
...
using CTCloud.SDK.Core.Auth;
// Import the specified {Service}, take Vpc as an example
using CTCloud.SDK.Vpc.V2;
using CTCloud.SDK.Vpc.V2.Model;
...
the [CHANGELOG.md](https://github.com/ctcloud-sdk/ctcloud-sdk-net/blob/master/CHANGELOG.md).

## User Manual [:top:](#ct-cloud-net-software-development-kit-net-sdk)
...
``` csharp
config.ProxyHost = "https://proxy.ru:8080";
config.ProxyUsername = "test";
config.ProxyPassword = "test";
```
...
String endpoint = "https://vpc.ru-moscow-1.hc.sbercloud.ru";
...
info: System.Net.Http.HttpClient.SdkHttpClient.LogicalHandler[100]
      Start processing HTTP request GET https://vpc.ru-moscow-1.hc.sbercloud.ru/v1/076958154900d2492f8bc0197405c803/vpcs?limit=1
info: System.Net.Http.HttpClient.SdkHttpClient.ClientHandler[100]
      Sending HTTP request GET https://vpc.ru-moscow-1.hc.sbercloud.ru/v1/076958154900d2492f8bc0197405c803/vpcs?limit=1
